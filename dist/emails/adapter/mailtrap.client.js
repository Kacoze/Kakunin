...
class MailTrapClient {
    ...
    async getEmails() {
        ...
        for (const message of messages) {
            const rawBody = await this.requestClient(`${config.endpoint}${message.raw_path}?api_token=${config.apiKey}`).then(res => res.text());
...
exports.create = (requestClient = fetch, config = config_helper_1.default.email) => {
