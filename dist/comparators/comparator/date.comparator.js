...
exports.supportedFormats = ['DD-MM-YYYY', 'DD-MM-YY', 'DD/MM/YYYY', 'DD/MM/YY'];
const isValidDate = date => {
    for (const format of exports.supportedFormats) {
...
exports.DateComparator = {
    ...
    compare: (values, order) => {
        for (let i = 1; i < values.length; i++) {
            ...
            const date = values[i];
            const foundPrevious = moment(datePrevious, exports.supportedFormats.find(format => moment(datePrevious, format).isValid()));
            const found = moment(date, exports.supportedFormats.find(format => moment(date, format).isValid()));
